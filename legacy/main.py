...
class ScoutWorkspace(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        self.route_panel.advanced_params_applied.connect(self.on_route_advanced_params_applied)
    ...
    def setup_docks(self):
        ...
        # --- DROITE : Thème ---
        self.dock_theme = QDockWidget("Bibliothèque de thèmes", self)
    ...
    def on_map_clicked(self, lat, lon):
        ...
        if self._stage_anchor_idx >= 0:
            ...
            # After one insertion click, resume normal extension from the end.
            # This prevents repeated unintended insertions in the middle.
            new_idx = len(stages) - 1
            self._stage_anchor_idx = new_idx
            tail = stages[new_idx]
            self.map_view.show_dashed_preview(tail["lat"], tail["lon"], new_idx)
    ...
    def set_stage_anchor(self, stage_idx):
        """Force anchor on a stage without toggle behavior."""
        stages = self.state_manager.get_state("stages", [])
        if stage_idx < 0 or stage_idx >= len(stages):
            return
        self._stage_anchor_idx = stage_idx
    ...
    def _calculate_route_for_stages(self, stages):
        ...
        small_roads = self.state_manager.get_state("small_roads_only", True)
    ...
    def _on_parallel_route_done(self, stages, leg_routes, all_coords):
        ...
        danger_scores = {"extreme": 4, "motorway_cross": 3, "high": 2, "minor": 1}
        ...
        for idx, (leg_name, leg_key, leg_geom) in enumerate(leg_routes):
            ...
            # Detect constrained/fallback legs on the whole itinerary.
            props = leg_geom.get("properties", {})
            if props.get("is_fallback", False):
                dl = props.get("danger_level", "minor")
                if dl in danger_scores:
                    fallback_warnings.append(leg_name)
                    if (max_danger_level is None) or (danger_scores.get(dl, 0) > danger_scores.get(max_danger_level, 0)):
                        max_danger_level = dl
        ...
        force_inter = bool(settings.get("force_intersections", True))
        current_leg_keys = {leg_key for _, leg_key, _ in leg_routes}

        polygonal_legs = self.state_manager.get_state("polygonal_legs", {})
        # Drop cached/pending legs that no longer belong to the current itinerary.
        polygonal_legs = {k: v for k, v in polygonal_legs.items() if k in current_leg_keys}
        self.state_manager.update_state("polygonal_legs", polygonal_legs)

        # Rebuild pending list from scratch for this route snapshot.
        # This avoids stale "pending" keys that can leave the loading pill spinning forever.
        pending_legs = []
        new_jobs = 0
        
        # Determine exactly which legs require calculation
        for idx, (leg_name, leg_key, leg_geom) in enumerate(leg_routes):
            if leg_key not in polygonal_legs:
        ...
        if pending_legs:
            self.statusBar().showMessage("Itinéraire dessiné. Calcul des azimuts...", 4000)
            self.map_view.show_loading("Calcul des azimuts...")
        else:
            self.map_view.hide_loading()
            if new_jobs == 0:
                self.statusBar().showMessage("Itinéraire dessiné.", 3000)
        ...
        if fallback_warnings:
            # Autoroute (trajet le long ou croisement) must always be shown as top-priority red warning.
            if max_danger_level in ("extreme", "motorway_cross"):
            ...
            alert_level = "extreme" if max_danger_level in ("extreme", "motorway_cross") else (max_danger_level or "warning")
            self.map_view.show_status_message(warn_msg, alert_level)
    ...
    def _apply_advanced_params(self, params):
        self.state_manager.push_to_history()
        poly_settings = {
            "allow_offroad": bool(params.get("allow_offroad", False)),
            "force_intersections": bool(params.get("force_intersections", True)),
            "tolerance": int(params.get("tolerance", 45)),
            "min_dist": int(params.get("min_dist", 80)),
            "bypassed": False,
        }
        self.state_manager.update_state("polygonalization_settings", poly_settings)
        self.state_manager.update_state("small_roads_only", bool(params.get("small_roads_only", True)))
    ...
    def on_route_advanced_params_applied(self, params):
        try:
            self._apply_advanced_params(params or {})
        except Exception as e:
            self.logger.error(f"Error applying route advanced params: {e}")

    def on_advanced_params_applied(self, params_str):
        import json
        try:
            params = json.loads(params_str)
            self._apply_advanced_params(params)
        except Exception as e:
    ...
    def refresh_ui(self):
        ...
        if not stages:
            self._stage_anchor_idx = -1
        elif getattr(self, "_stage_anchor_idx", -1) >= len(stages):
            self._stage_anchor_idx = len(stages) - 1
    ...
    def _apply_workspace(self, preset_id):
        ...
        if preset_id == "complet":
            # All docks visible
            for d in [self.dock_route, self.dock_modules, self.dock_difficulty]:
                d.setVisible(True)
            if hasattr(self, "dock_segmentation"):
