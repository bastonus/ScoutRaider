...
class Bridge(QObject):
    ...
    userGeolocationReported = Signal(float, float)
    ...
    @Slot(float, float)
    def onUserGeolocation(self, lat, lon):
        self.userGeolocationReported.emit(lat, lon)
...
class MapView(QWidget):
    ...
    def __init__(self, state_manager):
        ...
        # Route web geolocation ("Me localiser" button) back to Python state.
        self.bridge.userGeolocationReported.connect(self._on_bridge_geolocation)
    ...
    def _start_native_geolocation(self):
        """Legacy native geolocation entrypoint (disabled; web geolocation is canonical)."""
        return

    def _on_bridge_geolocation(self, lat, lon):
        """Receive web geolocation and update both async listeners and map state."""
        self._geo_signal.emit(lat, lon)

    def _apply_native_geolocation(self, lat, lon):
        """Handle native position update and center the map."""
        self._last_known_user_pos = (lat, lon)
    ...
    def request_user_location(self):
        """Trigger a fresh location request from both map UI and native backend."""
    ...
    def render_segments(self, segments, assignments, violations=None, fit_bounds=True, geojson=None, danger_pois=None):
        ...
        total_route_dist_m = 0.0
        for r in routes:
            try:
                total_route_dist_m += float(r.get("distance_m", 0) or 0)
            except Exception:
                pass
        ...
        js = f"if (typeof renderSegments === 'function') {{ renderSegments({json.dumps(segments)}, {json.dumps(assignments)}, {'true' if show_arrows else 'false'}, {fit_js}, {json.dumps(stages)}, {vios_js}, {geo_js}, {poi_js}, {json.dumps(total_route_dist_m)}); }}"
        self.web_view.page().runJavaScript(js)
