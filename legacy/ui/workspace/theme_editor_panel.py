...
from PySide6.QtWidgets import (
    ...
    QScrollArea, QWidget, QMessageBox, QSplitter, QFormLayout, QFileDialog
)
...
class ThemeEditorDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        row_icon.addWidget(self.input_icon, 1)
        btn_icon = QPushButton("Parcourir...")
        btn_icon.clicked.connect(self._browse_icon)
        row_icon.addWidget(btn_icon)

        row_preview = QHBoxLayout()
        row_preview.setContentsMargins(0, 0, 0, 0)
        row_preview.addWidget(self.input_preview, 1)
        btn_preview = QPushButton("Importer image...")
        btn_preview.clicked.connect(self._browse_preview)
        row_preview.addWidget(btn_preview)
        
        flay_gen.addRow("Nom du thème:", self.input_name)
        flay_gen.addRow("Description (cartes):", self.input_description)
        flay_gen.addRow("Icône (svg/png):", row_icon)
        flay_gen.addRow("Image preview:", row_preview)
    ...
    def _populate_form_from_dict(self, key):
        ...
        self.input_description.setText(t.get("description", ""))
        self.input_poi.setText(t.get("poi", ""))
        self.input_vigenere.setText(t.get("vigenere_key", ""))
        self.input_icon.setText(t.get("icon", ""))
        self.input_preview.setText(t.get("preview_image", ""))
    ...
    def _save_current_form_to_dict(self):
        ...
        new_data = {
            ...
            "preview_image": self.input_preview.text().strip(),
    ...
    def _browse_icon(self):
        path, _ = QFileDialog.getOpenFileName(
            ...
            os.path.join(PROJECT_ROOT, "ui", "workspace", "icons"),
            "Icônes (*.svg *.png *.jpg *.jpeg *.webp);;Tous les fichiers (*.*)"
        )
        if not path:
            return
        self.input_icon.setText(self._to_project_relative(path))

    def _browse_preview(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Importer une image preview",
            os.path.join(PROJECT_ROOT, "assets"),
            "Images (*.png *.jpg *.jpeg *.webp *.gif);;Tous les fichiers (*.*)"
        )
        if not path:
            return
        self.input_preview.setText(self._to_project_relative(path))

    @staticmethod
    def _to_project_relative(path):
        try:
